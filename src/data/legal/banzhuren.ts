    ...
    privacy: {
      ...
      sections: [
        ...
        },
        {
          title: "保存期限、撤回与删除请求",
          paragraphs: [
            "本地业务资料，包括工作留痕的经纬度、位置文本、拍摄照片和其他附件，会保留到你在应用内删除相应记录、清空数据或卸载应用为止；用于防止重复试用的首次启动日期 Keychain 标记按前述规则保留。可选 iCloud 备份中的数据由你的 Apple 账户和 Apple 的 CloudKit 规则管理。",
    ...
    terms: {
      ...
      sections: [
        ...
        },
        {
          title: "系统服务、购买与网络",
          paragraphs: [
            "通知、语音识别、相机、照片导入和保存、定位与反向地理编码、StoreKit、Apple ID、可选 iCloud 备份、系统邮件以及 timor.tech 节假日数据等能力依赖 Apple 或其他系统/网络服务。只有当你主动使用对应功能时，应用才会调用相机或处理工作留痕的经纬度与位置文本。相关服务可能因设备设置、系统版本、网络状态或服务方变更而不可用。一次性“终身解锁”商品的付款、退款和账户交易由 Apple 按其规则处理。",
