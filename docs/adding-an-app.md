...
`npm run optimize:media` generates WebP files under each app's `web/` directory without altering approved source images. `npm run validate` blocks missing media, localized catalog copy, legal documents, contact addresses, or App Store listing data. `npm run refresh:storefronts` checks the live Apple Lookup results for currently released Storefronts and prints any name or URL update that needs a review.
