...
站点对外联系邮箱统一为所有者指定的 `fxcpxs@163.com`。这项规则覆盖应用详情页、隐私政策、用户支持、服务条款、页脚、来源映射和新增应用文档；历史来源中出现的旧邮箱只作为迁移溯源信息，不得继续出现在已发布站点中。
...
- `docs/adding-an-app.md` 定义并约束新增应用流程。新增记录必须提供五语介绍、五个目标商店的已核实名称/URL或不可用状态、真实图标与截图、至少五项由当前产品行为证实的功能说明、统一公开邮箱，以及五语隐私/支持/条款内容。
